...
maxsCtr    = np.array([ 0.5, 0.5, 5.0,np.pi])
...
if model == "KingMS":
	from ModelsEll.KingMS import Module,Number,Density
	#--------- Initial parameters --------------
	namepar = ["$r_ca$", "$r_ta$","$r_tb$", "slope"]
	mins    = np.array(([ 0.0, 10.0,10.0,0]))
	maxs    = np.array(([ 5.0, 50.0,50.0,1]))
	paramsR = [2.0,20.0,15.0,0]
...
if rank ==0:
	if real :
		...
		cdtsT = np.array(data[:,[1,2,32,17]],dtype=np.float32)
		fdata = dir_+'Data/Members-0.84.csv'
		data  = np.array(read_csv(fdata,header=0,sep=','))
		cdtsD = np.array(data[:,[10,11,8,5]],dtype=np.float32)
