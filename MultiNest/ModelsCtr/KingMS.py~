...
#            print(mock_params[i])
        ...
        mock_params = mock_params[idx,:]

        #------ Computes Likelihood -----------
        llike = np.sum(map(lambda x,thetaparams:LogLikeStar(x,thetaparams,self.Rmax,self.sg),data,mock_params))
#        llike = np.sum(map(lambda x:LogLikeStar(x,params,self.Rmax,self.sg),data))

#        tmp1 = map(lambda x,thetaparams:LogLikeStar(x,thetaparams,self.Rmax,self.sg),data,mock_params)
#        for i in range(len(tmp1)):
#            print(mock_params[i],tmp1[i])
#        print(llike)
